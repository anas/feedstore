...
class Address {
	...
	public function copy($temp){
		//This function copies the values in the $temp object to the $this object
		//AND saves it to the database
		$this->setCity($temp->getCity());
		$this->setCountry($temp->getCountry());
		$this->setPostalCode($temp->getPostalCode());
		$this->setState($temp->getState());
		$this->setStreetAddress($temp->getStreetAddress());
		$this->save();
	}
	...
	public function getAddEditForm($name = "") {
		...
		$form->setConstants( array ( 'state' => '1' ) );//31 is the ID of Alberta
		$form->addElement( 'hidden', 'state' );
		$form->addElement('static', 'state1', 'State / Province','Alberta');

		$form->setConstants( array ( 'country' => '31' ) );//31 is the ID of Canada
