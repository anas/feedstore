...
abstract class DBColumn {
	...
	*/
	...
	function __construct($name=null, $label = null, $modifier = "", $options = null) {
		$this->_name = $name ? $name : $this->type();
		$this->_label = $label ? $label : ucwords(str_replace('_', ' ', $this->name())); 
	...
	static function make($fullType, $name=null, $label=null, $options = null, $modifier = null) {
		preg_match ('/^([!?\/]*)([a-zA-Z_]+)(\(.*\))?([!?\/]*)/', $fullType, $matches);
		$type = $matches[2];
